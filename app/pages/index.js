...
export default function Home() {
  ...
  const loadWaitlistStats = async () => {
    try {
      ...
      // Keep the default count of 0 if stats can't be loaded
    }
  };
    ...
    try {
      ...
      if (data.success) {
        ...
        if (errorMessage.includes('Service temporarily unavailable')) {
          showError('We\'re experiencing technical difficulties.\nPlease try again in a few minutes.');
        } else {
                  ...
                  <div className="flex gap-2">
                    <div className="flex-1">
                      <input
                        ...
                        className={`w-full h-9 px-2 py-1 text-sm rounded-md border text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring focus:border-transparent transition-colors ${inputError
                          ? 'border-red-500 focus:ring-red-500 focus:border-red-500 bg-red-500/5'
                          : 'border-border bg-input'
                          } ${shakeInput ? 'input-error-shake' : ''}`}
                        disabled={isSubmitting}
                      />
                      {statusMessage && (
                        <div className={`text-sm px-3 py-2 mt-4 rounded-md border transition-all duration-300 ${statusType === 'success'
